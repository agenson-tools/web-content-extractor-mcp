    ...
    try {
        switch (name) {
            ...
            case 'extract_structured_data': {
                const { url, data_types = ['tables', 'lists'], options = {} } = args;
            ...
            case 'batch_extract': {
                const { urls, extraction_type = 'article', options = {} } = args;
...
async function extractArticleContent(url, options) {
...
async function extractStructuredData(url, dataTypes, options) {
    const startTime = Date.now();
    try {
        let html;
        if (options.javascript_enabled) {
        ...
        if (dataTypes.includes('tables')) {
            const tables = Array.from(document.querySelectorAll('table')).map((table, index) => {
                ...
                return {
                    ...
                    context: options.include_context ? getElementContext(table) : null
                };
            });
        ...
        if (dataTypes.includes('lists')) {
            const lists = Array.from(document.querySelectorAll('ul, ol')).map((list, index) => {
                ...
                return {
                    ...
                    context: options.include_context ? getElementContext(list) : null
                };
            });
...
async function extractLinks(url, filterOptions) {
    const startTime = Date.now();
    try {
        const response = await fetch(url);
        ...
        const links = Array.from(document.querySelectorAll('a[href]')).map(link => {
            ...
            try {
                ...
                return {
                    ...
                    text: cleanText(text),
                    type: linkType,
                    context: filterOptions.include_context ? getElementContext(link) : null
                };
            }
            catch {
...
async function screenshotToMarkdown(url, options) {
    const startTime = Date.now();
    try {
        const browser = await getBrowser();
        const page = await browser.newPage();
...
async function batchExtract(urls, extractionType, options) {
    const startTime = Date.now();
...
function generateLayoutMarkdown(layoutInfo) {
